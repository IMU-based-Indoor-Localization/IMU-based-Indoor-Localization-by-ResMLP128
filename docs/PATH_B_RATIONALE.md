...
- P64 에서 직사각형 25×12 m GT 실측으로 **단일 scalar 한계 정량 확정**:
  ...
  **bbox vs 실제 변 길이 정정** (2026-05-24, 4 모서리 검출):
  bbox 는 회전된 직사각형의 *외접* 사각형 → 실제 변 보다 큼.
  실제 변별 분석 결과:

  | 측정 | 긴 변 | 짧은 변 | 긴/GT | 짧/GT | 비율 (긴/짧, GT=2.08) |
  |---|---:|---:|---:|---:|---:|
  | scale 2.5 (P60) | 10.80 m | 5.82 m | 43.2% | 48.5% | 1.85 |
  | scale 2.0 (P64) | 7.46 m | 3.85 m | 29.8% | 32.1% | 1.94 |
  | **P67 적응 scale** | **12.99 m** | **7.20 m** | **52.0%** | **60.0%** | 1.80 |

  → **양쪽 변 모두 균일 과소** (이전 *짧은 변 98%* 결론은 bbox 착시).
    그러나 변 비율 (1.80~1.94) 이 GT (2.08) 보다 작음 = saturation 의
    잔흔 (긴 변 = 빠른 보행 = 학습 over-range → 모델 더 압축).
...
- P64 결정: scale **2.0** 로 절충 (긴 변 30% / 짧은 변 32%, 양쪽 일관 과소)
- **P67 / P67-B 임계 기반 적응 scale 도입** (2026-05-24)
  - 단일 scalar → 4-구간 조각별 선형 보정
  - P67  임계 [0.15, 0.30, 0.45] / 값 [1.0, 2.0, 3.5, 5.5]
  - P67-B 임계 [0.15, 0.25, 0.40] / 값 [1.0, 2.5, 5.0, 7.0]  ← **현재 채택**
  - 진화 (같은 직사각형 25×12m GT):

    | 측정 | 긴 변 | 짧은 변 | 긴/GT | 짧/GT | 비율 (GT=2.08) |
    |---|---:|---:|---:|---:|---:|
    | scale 2.5 (P60) | 10.80 m | 5.82 m | 43% | 49% | 1.85 |
    | scale 2.0 (P64) | 7.46 m | 3.85 m | 30% | 32% | 1.94 |
    | P67 적응 | 12.99 m | 7.20 m | 52% | 60% | 1.80 |
    | **P67-B 적응** | **21.05 m** | **11.50 m** | **84%** | **96%** | **1.83** |

  - P67-B 둘레 70.28m / GT 74m = **95%**, drift 9.0% (RoNIN baseline 양호)
  - scale hit: 5.0× 79% (정상), 7.0× 12% (빠른), 2.5× 7% (느린), 1.0× 2% (정지)
  - **잔존 변 비율 1.83 vs GT 2.08 (88% 도달) = saturation 본질의 한계**.
    더 큰 scale 로도 못 줄임 (정지 발산 위험). 모델 fine-tuning 으로만 해결.
