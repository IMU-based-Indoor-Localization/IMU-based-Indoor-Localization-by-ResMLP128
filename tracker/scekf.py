...
class State(object):
    ...
    def apply_correction(self, dX):
        ...
        assert dX_past.flatten().shape[0] == self.N * 6, (
            f"past error states {dX_past.flatten().shape[0]} != {self.N * 6}"
        )

        if self.N > 0:
            temp = dX_past.reshape((self.N, 6))
            dps = np.expand_dims(temp[:, 3:6], axis=2)
            dthetas = temp[:, 0:3]
            dRs = mat_exp_vec(dthetas)
...
def propagate_covariance(A_aug, B_aug, dt, Sigma, W, Q):
    dim_new_state = A_aug.shape[0] - A_aug.shape[1]
    ...
    ret[: -15 - dim_new_state, : -15 - dim_new_state] = Sigma[:-15, :-15]
    ret[: -15 - dim_new_state, -15 - dim_new_state :] = Sigma[:-15, -15:] @ AT
    ret[-15 - dim_new_state :, : -15 - dim_new_state] = A @ Sigma[-15:, :-15]
    ret[-15 - dim_new_state :, -15 - dim_new_state :] = A @ Sigma[-15:, -15:] @ AT
    ret[-15 - dim_new_state :, -15 - dim_new_state :] += B_aug @ W @ B_aug.T
    ret[-6:, -6:] += dt * Q
    return ret
...
class ImuMSCKF:
    def __init__(self, config=None):
        expected_attribute = [
            "sigma_na", "sigma_ng", "ita_ba", "ita_bg",
            "init_attitude_sigma", "init_yaw_sigma", "init_vel_sigma",
            "init_pos_sigma", "init_bg_sigma", "init_ba_sigma",
            "mahalanobis_fail_scale",
        ]
        if not all(hasattr(config, attr) for attr in expected_attribute):
            logging.warning("At least one filter parameter will use its default value.")
    ...
    def initialize(self, t_us, acc, ba_init, bg_init):
        ...
        logging.info("filter initialized with gravity alignment!")

    def get_past_state(self, t_us):
        assert isinstance(t_us, int)
        state_idx = self.state.si_timestamps_us.index(t_us)
        return self.state.si_Rs[state_idx], self.state.si_ps[state_idx]
    ...
    def get_evolving_state(self):
        return (
            ...
            self.state.s_v.copy(),
            self.state.s_p.copy(),
            self.state.s_ba.copy(),
            self.state.s_bg.copy(),
        )

    def get_covariance(self):
        return self.Sigma, self.Sigma15
    ...
    def is_mahalanobis_activated(self):
        ...
        if self.last_success_mahalanobis is not None:
            if self.state.s_timestamp_us > self.last_success_mahalanobis + int(0.5 * 1e6):
                logging.warning("Deactivating Mahalanobis test, failed for too long.")
                self.last_success_mahalanobis = None
    ...
    def propagate(self, acc, gyr, t_us, t_augmentation_us=None):
        ...
        self.state.unobs_shift = A_aug @ self.state.unobs_shift
    ...
    def update(self, meas, meas_cov, t_begin_us, t_end_us):
        ...
        try:
            ...
            raise RuntimeError(f"EKF update: timestamps {t_begin_us}, {t_end_us} not found")

        self.R = self.meascov_scale * meas_cov
        if self.use_const_cov:
            self.R = self.meascov_scale * np.diag(
                np.array([self.const_cov_val_x, self.const_cov_val_y, self.const_cov_val_z])
            )
        ...
        ri_eul = compute_euler_from_matrix(Ri, "xyz", extrinsic=True)
        ri_y = ri_eul[0, 1]
        ri_z = ri_eul[0, 2]
        Ri_z = np.array(
            [
        ...
        if abs(np.cos(ri_y)) < 1e-5:
            logging.warning("Singularity in H matrix, dropping update")
            return

        assert self.Sigma.shape[0] == H.shape[1]
