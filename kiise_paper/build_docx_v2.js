...
function imgPara(file, w) {
  ...
  const dims = { "fig1_pipeline.png":[1444,663],"fig_app.png":[1514,1490],"fig_headline_bar.png":[1409,584],"fig2_backbone.png":[2262,991],"fig3_handbag.png":[2100,900],
...
push(CAP("그림 2. 온디바이스 앱 구현 화면 — (a) 실시간 측위 궤적·위치·속도와 추론 지연 약 13 ms, (b) 미래관 3층 평면도 오버레이(실측 보행 수집 현장; 일부 실명 블러)."));
...
push(CAP("그림 3. Handbag 세 모델 예측 경로 비교. ResMLP128 1.250 m로 최저 오차."));
...
push(P("트롤리 외 전 카테고리에서 EKF가 열등하며, 격한 동작에서 3~5배, 장거리에서 8배 이상 악화된다(표 5, 그림 4·5)."));
...
push(imgPara("fig_headline_bar.png", 540));
push(CAP("그림 4. 카테고리별 네트워크 단독 vs EKF 결합 측위 오차(RMSE, 로그축). trolley만 EKF 우위(+17%)이고 나머지는 2.6~8.2배 악화."));
push(imgPara("fig4_handheld.png", 470));
push(CAP("그림 5. Handheld 대표 시퀀스 30초 GT·Net·EKF 궤적(Net 0.230→EKF 2.802 m). 전체 시퀀스에서도 EKF 오차가 네트워크의 5.0배(표 5)."));
...
push(P("배포 단말(Galaxy S23 FE) 실내 보행 3개 시퀀스에 대한 정성적 사례 검토(case study, N=3)이며, 정량 본체는 OxIOD 152 시퀀스다."));
push(P([T("(a) 모델 in-domain 강건성. ",{bold:true}), T("OxIOD 7개 카테고리에서 dead-reckoning 경로 길이가 GT와 거의 일치하고 ATE가 경로 길이의 약 1%로(표 10), Android 스케일 이탈은 모델이 아니라 학습–배포 도메인 시프트에서 비롯됨을 보인다.")]));
push(CAP("표 10. 배포 모델의 OxIOD in-domain dead-reckoning ATE(경로 길이 약 1%)."));
push(makeTable([
  ["카테고리","시간","DR경로","GT경로","ATE(m)","rel%"],
  ...
  ["running","691s","968 m","941 m","4.60","0.5%"],
  ["평균","—","—","—","2.49","약 1%"],
], [2772,1400,1700,1700,1500,1700]));
push(P([T("(b) 전처리 정렬의 실측 재현. ",{bold:true}), T("프레임만 ga/window-start/body로 바꿔 입력 전처리 효과만 격리하면, 3개 보행 모두 ga≈window-start(≤0.95 m)이고 body는 발산(1.7–15.3 m)하여 4.6절 정렬 결과를 실측 도메인에서 재현한다(표 11).")]));
push(CAP("표 11. 실측 Android 정렬 재현: ga≈window-start, body는 발산(N=3)."));
push(makeTable([
  ["보행(경로)","ga ↔ window-start","ga ↔ body"],
  ["A (9 m)","0.29 m","1.68 m"],
  ["B (23 m)","0.95 m","10.63 m"],
  ["C (30 m)","0.36 m","15.32 m"],
], [3772,3500,3500]));
push(P([T("(c) yaw 드리프트 민감도의 실측 재현. ",{bold:true}), T("합성 yaw 드리프트를 실측 자세에 주입하면 변위 크기(경로 길이)는 불변하고 방향(끝점 편차)만 누적 yaw에 비례 증가하여 OxIOD와 일치한다(표 12). 본 단말 자이로-only heading은 20–33초에 72–113° 드리프트하여 RotVec 채택을 실측으로 정당화한다.")]));
push(CAP("표 12. 실측 Android yaw 드리프트 재현: 크기 불변, 방향은 누적 yaw에 비례 증가(N=3, 누적≈165°)."));
push(makeTable([
  ["보행","시간·순변위","자이로 드리프트","크기","끝점편차"],
  ["A","20s·5.3 m","77.7° (3.8°/s)","8.9 m","3.8 m"],
  ["B","33s·4.9 m","113.2° (3.4°/s)","22.9 m","14.2 m"],
  ["C","33s·17.9 m","72.5° (2.2°/s)","29.7 m","30.2 m"],
], [1800,2972,2800,1600,1600]));
...
Packer.toBuffer(doc).then(buf => {
  fs.writeFileSync(path.join(DIR, "KIISE_paper_v2.docx"), buf);
  console.log("[OK] KIISE_paper_v2.docx written");
});
