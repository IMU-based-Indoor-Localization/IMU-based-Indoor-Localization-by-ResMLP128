...
# ── 정지 상태 감지 및 zero-displacement 업데이트 파라미터 ──────────────────────
# 자이로 RMS 임계값 (rad/s): 이 값 미만이면 정지로 판단.
# 정지 MEMS 자이로 노이즈 ≈ 0.003–0.01 rad/s, 보행 ≈ 0.1–0.5 rad/s.
# 0.05 rad/s: Android 기준과 일치 (정지 노이즈 5×, 느린 보행 이상은 통과).
GYR_STATIC_THRESHOLD = 0.05   # rad/s

# 정지 상태 zero-displacement 측정 공분산 (m²).
# std = sqrt(1e-4) ≈ 0.01 m = 1 cm → 네트워크 평균(exp(-2)≈0.13 m²) 대비 강한 제약.
STATIC_DISP_COV = 1e-4        # m²
...
class ImuTracker:
    ...
    def _process_update(self, t_us):
        ...
        if self.debug_callback_get_meas:
            ...
            # ── 정지 상태 감지: displacement window 구간의 자이로 RMS ────────────
            # net_gyr_w 는 [net_input_size, 3] 배열.
            # 뒤쪽 disp_window_size 샘플이 실제 변위 추정 구간.
            gyr_window = net_gyr_w[-self.disp_window_size:]          # [W, 3]
            gyr_rms = float(np.sqrt(np.mean(gyr_window ** 2)))
            if gyr_rms < GYR_STATIC_THRESHOLD:
                # 정지: 네트워크 대신 zero-displacement + 소형 공분산으로 EKF 고정
                meas = np.zeros((3, 1))
                meas_cov = STATIC_DISP_COV * np.eye(3)
                logging.debug(
                    f"STATIC 감지 (gyr_rms={gyr_rms:.4f} rad/s) — "
                    f"zero-displacement 업데이트 적용"
                )
            else:
                # 이동: 네트워크 추론 결과 사용
