...
def main():
    ...
    fig.suptitle(f"그림 4. 입력 전처리 정렬 ablation 궤적 비교 — {args.catA} ({seqA.name})", fontweight="bold")
    ...
    bx.set_title(f"그림 5. 입력 yaw 드리프트 궤적 (input_only) — {args.catB} ({seqB.name})", fontsize=10, fontweight="bold")
